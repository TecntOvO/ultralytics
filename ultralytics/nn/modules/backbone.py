...
class MConv2d(nn.Module):
    ...
    def __init__(self, c1, c2, k=3, s=1, groups=1, bias=False, norm=True, act=True):
        """Initialize Conv layer with given arguments including activation."""
        super().__init__()
        padding = (k - 1) // 2
        self.layers = nn.Sequential()
        self.layers.add_module('conv', nn.Conv2d(c1, c2, k, s, padding, groups=groups, bias=bias))
        if norm:
            self.layers.add_module('BatchNorm2d', nn.BatchNorm2d(c2))
        if act:
            self.layers.add_module('Activation', nn.ReLU6())
