...
class Annotator:
    ...
    def box_label(self, box, label="", color=(128, 128, 128), txt_color=(255, 255, 255), rotated=False):
        ...
        if self.pil or not is_ascii(label):
            ...
            if rotated:
                ...
                cv2.rectangle(blk, p1, p2, color, thickness=-1, lineType=cv2.LINE_AA)
                self.im =cv2.addWeighted(self.im, 1.0, blk, 0.1, 0)
            if label:
    ...
    def add_id_num(self,id_count):
        cv2.putText(
            self.im,
            f"Total ID: {id_count-1}",
            (10,25),
            0,
            0.85,
            (255,0,0),
